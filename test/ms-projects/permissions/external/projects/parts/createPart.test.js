...
describe('MSProjects – Permissions - create parts', () => {
	let accessTokenAdmin;
	let accessTokenAuthority;
	let accessTokenUser;
	let accessTokenBanned;

	let URL = '/projects-service/projects/fakeIdProject/parts';
	const access = [
		['admin', true],
		['authority', true],
		['user', true],
	...
	const genRequest = (token = null) =>
		client.post(URL, {}, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
